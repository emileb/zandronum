// ff65c02e5d2dc05fd12cdfce7d9f4eb5
	...
	case SVC2_SOUNDSECTOR:
		{
			...
			command.sector = CLIENT_FindSectorByID( temp147 );
			command.channel = bytestream->ReadShort();
			command.sound = bytestream->ReadString();
			command.volume = bytestream->ReadByte();
			command.attenuation = bytestream->ReadByte();
	...
	case SVC2_ACSSENDSTRING:
		{
			...
			if ( CLIENT_ReadActorFromNetID( temp149, RUNTIME_CLASS( AActor ), true,
											reinterpret_cast<AActor *&>( command.activator ),
	...
	case SVC2_SYNCJOINQUEUE:
		{
			...
			unsigned int temp150;
			temp150 = bytestream->ReadByte();
			command.slots.Reserve( temp150 );
			for ( unsigned int i = 0; i < temp150; ++i )
			{
	...
	case SVC2_SYNCMAPROTATION:
		{
			...
			unsigned int temp151;
			temp151 = bytestream->ReadByte();
			command.entries.Reserve( temp151 );
			for ( unsigned int i = 0; i < temp151; ++i )
			{
